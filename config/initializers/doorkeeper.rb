...
# Optional OAuth scopes. "openid" is always the default scope (required for OpenID Connect).
# Edit this list to control which scopes appear in the admin OAuth application form.
DOORKEEPER_OPTIONAL_SCOPES = %w[profile email].freeze
...
Doorkeeper.configure do
  ...
  optional_scopes *DOORKEEPER_OPTIONAL_SCOPES.map(&:to_sym)
  ...
  #
  # Enable logging for debugging OAuth flow (guarded to avoid nil contexts)
  before_successful_authorization do |controller, context|
    # Only available for the authorize endpoint
    next unless context&.respond_to?(:pre_auth) && context.pre_auth

    pre = context.pre_auth
    Rails.logger.info "=== Doorkeeper Authorization Request ==="
    Rails.logger.info "Client ID: #{pre.client.uid}"
    Rails.logger.info "Redirect URI: #{pre.redirect_uri}"
    Rails.logger.info "Scopes: #{pre.scopes}"
    Rails.logger.info "Code Challenge: #{pre.code_challenge}" if pre.code_challenge
    Rails.logger.info "Code Challenge Method: #{pre.code_challenge_method}" if pre.code_challenge_method
  end

  after_successful_authorization do |controller, context|
    # Only available for the authorize endpoint
    next unless context&.auth

    resp = context.auth
    Rails.logger.info "=== Doorkeeper Authorization Success ==="
    Rails.logger.info "Response: #{resp.inspect}"
    Rails.logger.info "Code: #{resp.code}" if resp.respond_to?(:code)
    Rails.logger.info "Redirect URI: #{resp.redirect_uri}" if resp.respond_to?(:redirect_uri)
  end

  # Log token endpoint successes (covers token exchange)
  after_successful_strategy_response do |request, response|
    Rails.logger.info "=== Doorkeeper Token Success ==="
    Rails.logger.info "Grant type: #{request.grant_type}"
    Rails.logger.info "Client ID: #{request.client&.uid}"
    Rails.logger.info "Scopes: #{request.original_scopes}" if request.respond_to?(:original_scopes)
  end
  ...
  #
  # This checks the `auto_approve` column on the oauth_applications table.
  # You can set this flag in the Trestle admin panel for trusted applications.
  #
...
# JWT access tokens (doorkeeper-jwt). Signed with RS256 using OIDC private key; public key for
# validation is at the OIDC discovery jwks_uri (same key as ID tokens).
# Skip when running assets:precompile (SECRET_KEY_BASE_DUMMY=1 in Dockerfile).
unless ENV["SECRET_KEY_BASE_DUMMY"].present?
  raise "OIDC_PRIVATE_KEY_PEM not set" unless defined?(OIDC_PRIVATE_KEY_PEM)
  ...
  Doorkeeper::JWT.configure do
    token_payload do |opts|
      ...
      aud = app.is_a?(Hash) ? app[:uid] : app&.try(:uid)
      payload[:aud] = aud if aud.present?
      if opts[:resource_owner_id].present?
        user = User.find_by(id: opts[:resource_owner_id])
        if user
          payload[:sub] = user.id
          payload[:user] = { id: user.id, email: user.email }
          payload[:permissions] = user.permissions.map { |p| { name: p.name, value: p.value } }
        end
      end
      payload
    end

    secret_key OpenSSL::PKey::RSA.new(OIDC_PRIVATE_KEY_PEM)
