...
| `JWT_SECRET` | Doorkeeper JWT access tokens. Generate: `openssl rand -hex 32` |

## Required for OpenID Connect

| Variable | Description |
|----------|-------------|
| `OIDC_PRIVATE_KEY` | RSA private key (PEM string) for signing OIDC ID tokens. In `.env`, use `\n` for newlines. Generate: `openssl genrsa 2048` |
...
- **Minimal production (Postgres):** `SECRET_KEY_BASE`, `JWT_SECRET`, `DATABASE_PASSWORD`, plus DB host/user if not localhost.
- **Standalone Docker (SQLite):** `SECRET_KEY_BASE`, `JWT_SECRET`, and `OIDC_PRIVATE_KEY`.
